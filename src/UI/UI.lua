...
function UI.Build(container, modules)
    ...
    -- 加载各个标签页
    local FunctionsTab = require(script.Tabs.FunctionsTab)
    local FFlagTab = require(script.Tabs.FFlagTab)
    local SensitivityTab = require(script.Tabs.SensitivityTab)
    local AccessoriesTab = require(script.Tabs.AccessoriesTab)
    local SettingsTab = require(script.Tabs.SettingsTab)
    local Notice = require(script.Parent.Notice)
    
    local tabs = {}
    
    tabs.functions = FunctionsTab.Create(ContentArea, C, Utils, modules.Toast, modules.Headless, modules.BrokenLegs, modules.Graphics, SwitchTab)
    tabs.functions.createContentFrame(ContentArea)
    tabs.functions.populate()
    
    tabs.fflag = FFlagTab.Create(ContentArea, C, Utils, modules.Toast, modules.FFlagTool)
    tabs.fflag.createContentFrame(ContentArea)
    tabs.fflag.populate()
    
    tabs.sensitivity = SensitivityTab.Create(ContentArea, C, Utils, Config, modules.Sensitivity, SavedConfig, sliderState)
    tabs.sensitivity.createContentFrame(ContentArea)
    tabs.sensitivity.populate()
    
    tabs.accessories = AccessoriesTab.Create(ContentArea, C, Utils, modules.Toast, modules.Accessories)
    tabs.accessories.createContentFrame(ContentArea)
    tabs.accessories.populate()
    
    tabs.settings = SettingsTab.Create(ContentArea, C, Utils, modules.Toast, D, SavedConfig, Panel, GetDefaultPos)
    tabs.settings.createContentFrame(ContentArea)
    tabs.settings.populate()
    
    -- 公告
    local notice = Notice.Create(Panel, C, Utils)
    NoticeBtn.MouseButton1Click:Connect(function()
        notice.setText("控制面板 v4.0\n\n模块化设计\n所有功能独立模块\nGitHub开源\n\n屏幕: " .. ScreenW .. "x" .. ScreenH)
        notice.show()
    end)
    ...
    return {
        ...
        Open = function()
            ...
            tabs.accessories.refreshAll()
            tabs.settings.updateInputs()
        end,
