...
function FFT.Create(parent, Colors, Utils, Toast, FFlagTool)
    ...
    local function populate()
        ...
        createBtn("应用FFlags", Colors.Success, function()
            local ok, r = FFlagTool.Apply(fflagInput.Text)
            if ok then Toast.Show("FFlag应用", "已应用 "..r.." 个", 2) else Toast.Show("FFlag错误", r, 2) end
        end)
        ...
        createBtn("加载上次", Colors.Warning, function()
            local s = FFlagTool.Load()
            if s then fflagInput.Text = s; Toast.Show("FFlag加载","已加载",2) else Toast.Show("FFlag","无保存文件",2) end
        end)
        
        task.delay(0.3, function()
            local s = FFlagTool.Load()
            if s then fflagInput.Text = s end
        end)
    end
